...
public class TestController extends Controller implements Initializable {
    ...
    private double getPercentOfAliveInfantryUnits(Army army, int startsUnits){
        return ((army.getInfantryUnits().size()* 100.0/startsUnits) );
    }
    private double getPercentOfAliveRangedUnits(Army army, int startsUnits){
        return  (army.getRangedUnits().size()* 100.0/startsUnits);
    }
    private double getPercentOfAliveCavalryUnits(Army army, int startsUnits){
        return ((army.getCavalryUnits().size()* 100.0/startsUnits));
    }
    private double getPercentOfAliveCommanderUnits(Army army, int startsUnits){
        return ((army.getCommanderUnits().size()* 100.0/startsUnits));
    }
    private double getPercentOfAliveMagicianUnits(Army army, int startsUnits){
        return ((army.getMagicianUnits().size()* 100.0/startsUnits));
    }
    private double getPercentOfAliveHealerUnits(Army army, int startsUnits){
